# CUDA_VISIBLE_DEVICES=4,7 OMP_NUM_THREADS=4 python -m torch.distributed.launch --master_port 29500 --nproc_per_node=2 train.py --id retrain_s4 --load_network /share22/home/tangquanhui/code/STCN/STCN/saves/stcn.pth --stage 4
...
def renew_inpainted_loader(max_skip):
    # our inpainted dataset is the same format as DAVIS2016
    inpainted_dataset = VOSDataset(path.join(inpainted_root, 'JPEGImages'), 
                        path.join(inpainted_root, 'Annotations'), max_skip, is_bl=False, subset=load_sub_davis())
    train_dataset = inpainted_dataset

    print('Inpainted dataset size: ', len(inpainted_dataset))
    print('Renewed with skip: ', max_skip)
...
if para['stage'] == 0:
    ...
    inpainted_root = path.join(path.expanduser(para['inpainted_root']), '2017', 'trainval')
    train_sampler, train_loader = renew_inpainted_loader(5)
